 ...
 * @author nowel
 */
class Search2Test extends PHPUnit2_Framework_TestCase {
    
    private $dao = null;
    
    public static function main() {
        $suite  = new PHPUnit2_Framework_TestSuite("Search2Test");
        $result = PHPUnit2_TextUI_TestRunner::run($suite);
    }

    protected function setUp() {
        $container = S2ContainerFactory::create(S2CONTAINER_PHP5_APP_DICON);
        $this->dao = $container->getComponent("search2.SearchDaoImpl");
    }
    ...
    public function testSearch5Find(){
        try {
            $this->dao->get5List();
    ...
    public function testSearch6Find(){
        $this->assertEquals(2, $this->dao->get6List()->size());
    }
    
    public function testSearch7Find(){
        try {
            $this->dao->get7List();
            $this->fail("Not Supported start with 'WHERE'");
        } catch(Exception $e){
